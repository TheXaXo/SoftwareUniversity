public class ExceptionMessages {
    public static final String EXAMPLE_EXCEPTION_MESSAGE = "Example message";

    public static final String DATA_ALREADY_INITIALIZED = "Data already initialized!";

    public static final String DATA_NOT_INITIALIZED = "Data is not already initialized!";

    public static final String NON_EXISTENT_COURSE = "Course does not exist!";

    public static final String NON_EXISTING_STUDENT = "User does not exist!";

    public static final String INVALID_PATH = "The path specified does not exist!";

    public static final String INVALID_FILTER = "The filter you are trying to use is invalid!";
}
