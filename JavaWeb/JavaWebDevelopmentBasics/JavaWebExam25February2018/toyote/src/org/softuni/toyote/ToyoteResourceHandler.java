...
public class ToyoteResourceHandler implements RequestHandler {
    ...
    private void retrieveResource(HttpRequest request, HttpResponse response) throws IOException {
        String resourceName = request.getRequestUrl().split("/")[request.getRequestUrl().split("/").length - 1];
        String resourceExtension = resourceName.split("\\.")[resourceName.split("\\.").length - 1];
