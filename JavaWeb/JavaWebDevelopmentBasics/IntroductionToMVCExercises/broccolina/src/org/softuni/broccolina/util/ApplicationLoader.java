...
public class ApplicationLoader {
    ...
    private void loadIfSolet(Class clazzFile, String applicationName) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        if (parentClass != null
                && Arrays.stream(parentClass.getInterfaces()).anyMatch(x -> x.getSimpleName().equals(HttpSolet.class.getSimpleName()))
                && !Modifier.isAbstract(clazzFile.getModifiers())) {
            Object soletObject = clazzFile.getConstructor().newInstance();
            ...
            if ((boolean) annotation.getClass()
            ...
            if (applicationName.equals(ROOT_APPLICATION_FOLDER_NAME)) {
                applicationName = "";
            } else {
                applicationName = "/" + applicationName;
            }

            String annotationRoute = annotation.getClass()
                    .getMethod("route")
                    .invoke(annotation)
                    .toString();

            this.loadedSoletsByApplicationName.putIfAbsent(applicationName + annotationRoute, soletObject);
        }
    }
    ...
    public void loadApplications() {
        ...
        try {
            ...
            for (Map.Entry<String, Object> pair : this.loadedSoletsByApplicationName.entrySet()) {
