...
public class RequestHandlerLoader {
        ...
        try {
            ...
            while (fileEntries.hasMoreElements()) {
                ...
                if (RequestHandler.class.isAssignableFrom(handlerClazz)) {
                    ...
                    //TODO find a better way to extract library name
                    this.loadedRequestHandlers.putIfAbsent(
                            library.getName()
                                    .substring(library.getName().lastIndexOf("\\") + 1)
                                    .replace(".jar", ""),
                            handlerObject);
                }
            }
    ...
    private void loadLibraries(String libFolderPath, Set<String> requestHandlersPriority) throws IOException {
        File libDirectory = new File(libFolderPath);

        if (libDirectory.exists() && libDirectory.isDirectory()) {
            List<File> libDirectoryJarFiles = Arrays.stream(libDirectory.listFiles())
                    .filter(x -> this.isLibraryFile(x))
                    .collect(Collectors.toList());

            for (String handlerPriority : requestHandlersPriority) {
                File currentFile = libDirectoryJarFiles.stream()
                        .filter(f -> f.getName().replace(".jar", "").equals(handlerPriority))
                        .findFirst()
                        .orElse(null);
    ...
    public void loadRequestHandlers(Set<String> requestHandlersPriority) {
        ...
        try {
            this.loadLibraries(LIB_FOLDER_PATH, requestHandlersPriority);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
